...
void man(char *name)
{
	...
	{
		printf("Description : Used to remove dat from file\n");
	...
	else if(strcmp(name,"closseall")==0)
	{
...
void DisplayHelp()
{
	...
	printf("fstat : To Display information of file usimg file descriptor\n");
	...
	while(1)
	{
		...
		else if(count == 3)
		{
			...
			else if(strcmp(command[0],"open")==0)
			{
				...
				if(ret >= 0)
					printf("File is succesfully opened with file descriptor\n");
				if(ret == -1)
					printf("ERROR : Incorrect parameters\n");
				if(ret == -2)
			...
			else if(strcmp(command[0],"read")==0)
			{
				...
				if(ret == -4)
					printf("It is not a regilar file\n");
				if(ret == 0)
					printf("File empty\n");
				if(ret > 0)
				{
